...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (isReadyForPull()) {
					...
					if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
						if (mMode.showHeaderLoadingLayout() && diff >= 40 && isReadyForPullStart()) {  //1f->26
							mLastMotionY = y;
							mLastMotionX = x;
						...
						} else if (mMode.showFooterLoadingLayout() && diff <= -40 && isReadyForPullEnd()) { //-1f->-26
							mLastMotionY = y;
							mLastMotionX = x;
							mIsBeingDragged = true;
							if (mMode == Mode.BOTH) {
	...
	public T getmRefreshableView() {
		return mRefreshableView;
	}

	public void setmRefreshableView(T mRefreshableView) {
